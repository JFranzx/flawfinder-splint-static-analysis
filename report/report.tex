...
The problem with strncpy is that it does not add the terminator character and strings without the terminator character can cause segmentation fault \cite{geeksstrcpy}.\\
snprintf, on the other hand, always adds the NULL terminator character,but in some older systems, its implementation is subject to the buffer overflow\cite{dangerc}.
